    ...
    return (
        <>
            ...
            <div className="grid grid-cols-3 gap-3">
                {LECList.map((CourseData) => <CourseCard ID={CourseData.ID} Title={CourseData.Title} Score={CourseData.Score} />)}
            </div>
            <h1 className="block antialiased tracking-normal font-sans text-xl font-semibold leading-snug text-blue-gray-900">
                CL
            </h1>
            <div className="grid grid-cols-3 gap-4">
                {LCList.map((CourseData) => <CourseCard ID={CourseData.ID} Title={CourseData.Title} />)}
            </div>
        </>
    );
}
